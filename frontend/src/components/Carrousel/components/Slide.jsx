...
function Slide({ img, isActive }) {
  return (
    <div
      className={`keen-slider__slide relative h-full ${!isActive ? "filter blur-sm" : ""}`}
    >
      <img
        src={img.src}
        ...
        >
          <div className="p-4 flex items-center justify-center  flex-col gap-2  bg-gradient-to-t from-black/70 to-black/70 text-white dark:from-black/90 bg-backdrop-blur-md h-[600px]">
            ...
            {img.cta && (
              <Link
                to={img.cta.link}
                className="mt-2 inline-block text-sm font-semibold rounded-3xl bg-blue-300 dark:bg-blue-100 hover:bg-blue-100 px-4 py-2 text-center"
              >
                {img.cta.label}
              </Link>
            )}
